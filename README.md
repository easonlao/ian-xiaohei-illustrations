...
把抽象判断、流程、状态和隐喻，变成带有统一小呆 IP 的纯白手绘配图语言。

小呆 IP（小火人形态） | 纯白手绘 | 少量红橙蓝批注 | Agent Skill / Prompt Pack | Layer 0

## 这个仓库是什么

这个仓库当前不是“固定 16:9 正文配图器”，而是“小呆”这一套视觉系统的 layer 0 底座。
...
它不是通用插画 prompt，也不是 PPT 信息图模板。它也不是某一个固定场景的成品 skill。当前仓库的职责是先把统一 IP、统一风格和统一隐喻方法打牢，后续再按场景扩展成长文横图、竖屏图文、视频流封面等上层模式。

默认视觉 IP 是「小呆」：纯手绘亮红色火焰形状实体剪影小人，脸上有一对白色小圆点眼睛，四肢为纤细的黑色火柴人线条。小呆不是吉祥物，不是贴纸，也不是站在角落里的装饰物，而是正在认真参与系统运转的荒诞工作者。

一句话：**先定义统一的小呆视觉系统，再让不同场景共用这套底座。**

## 适合谁用

特别适合：
- 想先建立统一 IP，再逐步适配不同内容场景的人
...
- 基础隐喻方法
- 一套可复用的底层提示词约束
- 面向上层场景继续扩展的 skill 基线

默认不输出：
- 已完整封装好的 article / feed / vertical 模式
- 固定单一画幅的最终策略
- 商业海报或封面 KV
- 大段文字型信息图
...
具体场景如长文横图、竖屏图文、视频流封面，建议在这一层之上继续增加独立的场景策略，而不是反过来让 layer 0 去绑定某一个固定画幅。
